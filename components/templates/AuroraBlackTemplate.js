// components/templates/AuroraBlackTemplate.js
...
const OpeningOverlay = ({ visible, onComplete }) => {
  ...
  useEffect(() => {
    if (visible) {
      ...
      setTimeout(() => { if (onComplete) onComplete(); }, 3800);
    }
  }, [visible, onComplete]);
    ...
    >
      {showText && (
        <div className={styles.openingContent}>
          <div className={styles.openingRing} />
          <div className={styles.openingText}>
            <div className={styles.openingTitle}>Aurora Wedding</div>
            <div className={styles.openingSubtitle}>A New Beginning</div>
          </div>
        </div>
      )}
    </div>
...
// 달력 컴포넌트 - Apple 미니멀 스타일
const AuroraCalendar = ({ targetDate }) => {
  ...
  return (
    <div className={styles.auroraCalendar}>
...
// 메인 컴포넌트
const AuroraBlackTemplate = ({ eventData = {}, categorizedImages = {}, allowMessages = false, messageSettings = {} }) => {
  ...
  useEffect(() => {
    ...
    const subscription = supabase
      .channel(`aurora_black_guest_book_${eventData.id}`)
  ...
  const defaultMessages = [
    { from: '민나', date: '2025.04.24 18:52', content: '결혼을 진심으로 축하합니다!\n항상 행복한 날들만 가득하길 바랍니다 💍' },
    { from: 'sooyeon', date: '2025.04.23 09:41', content: '두 분의 아름다운 사랑이 결실을 맺게 되어 정말 축하드려요🎉\n언제나 서로를 아끼고 사랑하는 행복한 부부가 되시길!' },
  ...
  useEffect(() => {
    ...
    const subscription = supabase
      .channel('aurora-black-guestbook-changes')
      .on('postgres_changes', { event: 'INSERT', schema: 'public', table: 'guest_book', filter: `event_id=eq.${eventData.id}` },
        (payload) => {
          const newMessage = { id: payload.new.id, from: payload.new.guest_name || '익명', phone: payload.new.guest_phone, date: fmtDate(payload.new.created_at), content: payload.new.message || '' };
      ...
      .on('postgres_changes', { event: 'UPDATE', schema: 'public', table: 'guest_book', filter: `event_id=eq.${eventData.id}` },
        (payload) => {
          const updatedMessage = { id: payload.new.id, from: payload.new.guest_name || '익명', phone: payload.new.guest_phone, date: fmtDate(payload.new.created_at), content: payload.new.message || '' };
  ...
  const checkAndShowWelcomeChoice = () => {
    ...
    if (localStorage.getItem(currentEventArrivalKey)) return false;
    if (verifiedPhone && !arrivalDismissed) {
      const hasPublic = guestMessages.some(msg => msg.phone === verifiedPhone);
      if (hasPublic) { localStorage.setItem(currentEventArrivalKey, 'true'); return false; }
      if (userChoice === 'contribution') { localStorage.setItem(currentEventArrivalKey, 'true'); return false; }
    ...
    <div className={styles.container}>
      ...
      <section className={styles.heroSection}>
        ...
        <div className={styles.heroOverlay}>
          <div className={styles.heroRingDecoration}>
            <div className={styles.heroRingOuter} />
            <div className={styles.heroRingInner} />
          </div>
          <div className={styles.heroContent}>
            <p className={styles.heroLabel}>AURORA WEDDING INVITATION</p>
            ...
            <p className={styles.heroVenue}>{eventData.location || '웨딩홀'}</p>
          </div>
        </div>
      </section>
      ...
      <section className={styles.dateBanner}>
        <div className={styles.dateBannerInner}>
          <div className={styles.dateBannerText}>{getEnglishDate()}</div>
          <div className={styles.dateBannerDot} />
          <div className={styles.dateBannerText}>{ceremonyTime}</div>
        </div>
      </section>
      ...
      <section className={styles.weddingDaySection}>
        ...
        <AuroraCalendar targetDate={eventData.date || eventData.event_date || '2025-10-04'} />
      ...
      <footer className={styles.footer}>
        <div className={styles.footerContent}>
          <div className={styles.footerGoldDeco} />
...
export default AuroraBlackTemplate;
