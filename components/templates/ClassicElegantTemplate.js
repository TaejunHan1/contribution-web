...
// 도어 오프닝 오버레이 (클래식 엘레강스)
const ClassicOpeningOverlay = ({ visible, onComplete }) => {
  ...
  useEffect(() => {
    if (visible) {
      setTimeout(() => setShowInner(true), 300);
      setTimeout(() => setOpenDoors(true), 800);
      setTimeout(() => setFadeOut(true), 2400);
      setTimeout(() => { if (onComplete) onComplete(); }, 3400);
    }
  }, [visible, onComplete]);
  ...
  return (
    ...
    >
      ...
      {/* Right door panel */}
      <div className={`${styles.doorRight} ${openDoors ? styles.doorOpen : ''}`} />
      {/* Center text */}
      {showInner && (
        <div className={`${styles.doorInner} ${openDoors ? styles.doorInnerVisible : ''}`}>
          <div className={styles.doorOrnament}>✦</div>
          <p className={styles.doorText}>We Cordially Invite You</p>
          <div className={styles.doorOrnamentLine} />
          <p className={styles.doorSubText}>to Our Wedding</p>
        </div>
      )}
    </div>
  );
...
const MainPhotoSlideshow = ({ images, onImagePress }) => {
  ...
  return (
    <div className={styles.classicPhotoContainer} onClick={() => onImagePress && onImagePress(currentIndex)}>
        ...
        images.map((image, index) => {
          ...
          return src ? (
            <img
              ...
              src={src}
              alt="Wedding"
              className={`${styles.classicPhoto} ${index === currentIndex ? styles.active : ''}`}
...
// 달력 컴포넌트 (클래식)
const ClassicCalendar = ({ targetDate }) => {
  ...
  return (
    <div className={styles.classicCalendar}>
      <div className={styles.calendarOrnament}>✦ ─── ✦</div>
  ...
  if (isExpired) {
    return (
      <div className={styles.weddingComplete}>
        ...
        <p className={styles.completeMessage}>오늘이 그날입니다. 축하합니다!</p>
      </div>
    );
  }
  return (
...
const ClassicElegantTemplate = ({ eventData = {}, categorizedImages = {}, allowMessages = false, messageSettings = {} }) => {
  ...
  const handleOpeningComplete = () => {
    ...
    const timer = setTimeout(() => { checkAndShowWelcomeChoice(); }, 500);
  ...
  useEffect(() => {
    ...
    let checkTimer;
    if (!showOpening) {
      checkTimer = setTimeout(() => { checkAndShowWelcomeChoice(); }, 1000);
    } else {
      checkTimer = setTimeout(() => {
        if (showOpening) { setShowOpening(false); setTimeout(() => { checkAndShowWelcomeChoice(); }, 500); }
      }, 4000);
    }
  ...
  return (
    <div className={styles.container}>
      <ClassicOpeningOverlay visible={showOpening} onComplete={handleOpeningComplete} />
      <FallingFlowers />

      {/* 인트로 섹션 */}
      <section className={styles.introSection}>
        <div className={styles.introOrnamentTop}>✦ ─────── ✦ ─────── ✦</div>
        <p className={styles.introSubtitle}>Wedding Invitation</p>
        <div className={styles.introOrnamentLine} />
        <div className={styles.introNames}>
          <div className={styles.introName}>
            <span className={styles.introNameKo}>{eventData.groom_name || '이민호'}</span>
            <span className={styles.introNameEn}>{groomEnglishName}</span>
          </div>
          <div className={styles.introAmpersand}>그리고</div>
          <div className={styles.introName}>
            <span className={styles.introNameKo}>{eventData.bride_name || '배하윤'}</span>
            <span className={styles.introNameEn}>{brideEnglishName}</span>
          </div>
        </div>
        <div className={styles.introOrnamentLine} />
        ...
        <div className={styles.introOrnamentBottom}>✦ ─────── ✦ ─────── ✦</div>
      </section>
        ...
        <div className={styles.coupleNamesSection}>
          <div className={styles.coupleNames}>
            <div className={styles.groomInfo}>
              ...
              <span className={styles.coupleParents}>{eventData.groom_father_name || '아버님'} · {eventData.groom_mother_name || '어머님'}의 아들</span>
            </div>
            <div className={styles.coupleAnd}>
              <span className={styles.coupleAndText}>&</span>
            </div>
            <div className={styles.brideInfo}>
              ...
              <span className={styles.coupleParents}>{eventData.bride_father_name || '아버님'} · {eventData.bride_mother_name || '어머님'}의 딸</span>
            </div>
          </div>
        </div>
      </section>
      ...
      <section className={styles.weddingDaySection}>
        ...
        <div className={styles.classicDateBlock}>
          <div className={styles.classicDateYear}>{dateInfo.year}</div>
          <div className={styles.classicDateDivider}>─────── ✦ ───────</div>
          <div className={styles.classicDateMonthDay}>{dateInfo.month}월 {dateInfo.day}일 {dateInfo.dayOfWeek}</div>
          <div className={styles.classicDateEng}>{getEnglishDate()}</div>
          <div className={styles.classicDateTime}>{ceremonyTime}</div>
        </div>
        <ClassicCalendar targetDate={eventData.date || eventData.event_date || '2025-10-04'} />
      ...
      <footer className={styles.footer}>
        <div className={styles.footerContent}>
          ...
          <div className={styles.footerNames}>{eventData.groom_name || '이민호'} ♡ {eventData.bride_name || '배하윤'}</div>
