-- Database Updates for Guest Book System (정리된 최종 버전)
-- 방명록 시스템을 위한 데이터베이스 업데이트

-- 1. guest_book 테이블 생성 (웹 방명록용)
...
-- 3. guest_book_relation_stats 테이블 (관계별 통계)
...
-- 4. public_guest_messages 테이블 (공개 메시지)
...
-- 8. 사용자-방명록 연결을 위한 뷰 (나중에 사용자가 가입할 때 활용)
