-- Database Updates for Guest Book System
-- 방명록 시스템을 위한 데이터베이스 업데이트

-- 1. sms_verifications 테이블에 필요한 컬럼 추가
...
-- 특정 핸드폰번호의 방명록 내역 조회 (사용자 가입 시 활용)
-- SELECT * FROM user_guest_history WHERE guest_phone = '+821012345678';
