...
export async function sendContributionNotification(eventId, contributionData) {
  try {
    ...
    // 4. 축의금 알림이 활성화되어 있는지 확인
    const notificationSettings = user.notification_settings || {};
    if (!notificationSettings.contribution) {
      console.log('축의금 알림이 비활성화됨');
      return { success: true, message: '사용자가 축의금 알림을 비활성화함' };
    }
    ...
    if (isExpoToken) {
      // Expo Push API 사용 (백그라운드 알림 지원)
      console.log('📱 Expo 토큰 감지 - Expo Push API 사용');
