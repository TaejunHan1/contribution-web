...
import {
  buildCategorizedImagesFromList,
  flattenCategorizedImages,
  hasCategorizedImages,
  normalizeCategorizedImages,
  parseJsonField,
} from './imageUtils';
...
const getEventImagePublicUrl = storagePath => {
  if (!storagePath || !supabase?.storage) return null;
...
const imageResolveOptions = {
  resolveStoragePath: getEventImagePublicUrl,
};
...
export const getEventDetails = async eventId => {
  try {
    ...
    const additionalInfo = parseJsonField(data.additional_info, {}) || {};

    // 🔥 이미지 데이터 처리 - DB에 저장된 최신 수정값을 우선 사용
    let processedImageUrls = [];
    let normalizedCategorizedImages = normalizeCategorizedImages(
      additionalInfo.categorized_images,
      imageResolveOptions
    );

    if (hasCategorizedImages(normalizedCategorizedImages)) {
      console.log('✅ 1순위: additional_info.categorized_images 최신 DB 이미지 사용');
      processedImageUrls = flattenCategorizedImages(normalizedCategorizedImages);
    } else if (Array.isArray(data.image_urls) && data.image_urls.length > 0) {
      console.log('🔍 2순위: DB image_urls 필드에서 이미지 처리...');
      normalizedCategorizedImages = buildCategorizedImagesFromList(data.image_urls, imageResolveOptions);
      processedImageUrls = flattenCategorizedImages(normalizedCategorizedImages);
    } else if (data.user_id) {
      console.log('🔍 3순위: Storage에서 eventId 기반 이미지 조회 시도...');
      ...
      if (storageImages.length > 0) {
        ...
        normalizedCategorizedImages = buildCategorizedImagesFromList(storageImages, imageResolveOptions);
        processedImageUrls = flattenCategorizedImages(normalizedCategorizedImages);
      }
    }
    ...
    const finalData = {
      ...
      additional_info: {
        ...additionalInfo,
        categorized_images: normalizedCategorizedImages,
      },
