...
export default function HomePage() {
  ...
  useEffect(() => {
    ...
    const checkScreenSize = () => {
      ...
      // 우선순위로 감지
      if ((width >= 280 && width <= 350) || width === 360 || width === 412) {
        setIsZFlip(true);
        ...
        <section className="bg-gradient-to-br from-blue-50 to-indigo-100 md:py-20 relative overflow-hidden">
          ...
          <div className="max-w-7xl mx-auto px-0 md:px-4 relative">
            ...
            <div className="block md:hidden text-center pt-8 pb-2">
              ...
              {/* 아이폰 프레임 - 모바일용 (더 큰 크기) */}
              <div className={`relative flex justify-center items-center w-full ${isZFlip ? '-mt-36 -mb-12' : isGalaxyFold ? '-mt-28 -mb-32' : isIPhoneX ? '-mt-16 -mb-4' : isSmallScreen ? 'mt-14 mb-2' : isIPhoneXR ? '-mt-10 mb-2' : isIPhone14ProMax ? '-mt-12 mb-2' : 'mb-2'}`}>
                <div className={`relative w-[95vw] drop-shadow-2xl ${isZFlip ? 'h-[500px] -mt-20' : isGalaxyFold ? 'h-[600px] -mt-8' : isIPhoneX ? 'h-[650px] -mt-24' : 'h-[700px] -mt-28'}`}>
                  ...
                  <div className="absolute inset-0 flex items-center justify-center z-5">
                    <div className={`w-[45%] ${isZFlip ? 'h-[25%]' : isGalaxyFold ? 'h-[52%]' : isIPhoneX ? 'h-[50%]' : isSmallScreen ? 'h-[66%]' : isGalaxyS8Plus ? 'h-[50%]' : 'h-[55%]'} rounded-[25px] overflow-hidden bg-white shadow-inner relative`}>
                      <iframe 
